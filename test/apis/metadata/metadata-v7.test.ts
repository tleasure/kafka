...
const { createRequest, parseResponse } = metadataV7
...
test('createRequest serializes request parameters correctly', () => {
  ...
  const writer = createRequest(topics, allowAutoTopicCreation)
...
test('parseResponse correctly processes a successful response', () => {
  ...
  const response = parseResponse(1, 3, 7, Reader.from(writer))
...
test('parseResponse throws error on topic error code', () => {
  ...
  throws(
    () => {
      parseResponse(1, 3, 7, Reader.from(writer))
    },
    (err: any) => {
